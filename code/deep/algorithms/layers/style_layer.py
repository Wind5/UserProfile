...
class StyleLayer(layer):
    ...
    def __init__(self, style_number, style_dim, hidden_status_dim, tparams=None, prefix='style'):
        ...
        self.W_BBeta = theano.shared(
            value=util.uniform_random_weight((style_dim, style_dim), 0.01),
            name=self._p(prefix, 'W_BBeta'),
            borrow=True
        )
        self.W_hBeta = theano.shared(
            value=util.uniform_random_weight((hidden_status_dim, style_dim), 0.01),
            name=self._p(prefix, 'W_hBeta'),
            borrow=True
        )
        self.B = theano.shared(
            value=util.uniform_random_weight((style_number, style_dim), 0.01),
            name=self._p(prefix, 'B'),
            borrow=True
        )
        self.vb = theano.shared(
                                util.uniform_random_weight((style_dim,), 0.01),
    ...
    def get_output(self, inputs):
        ...
        row_max_beta = beta.max(axis=1).dimshuffle(0, 'x')
        
        beta = T.nnet.softmax(beta - row_max_beta)
